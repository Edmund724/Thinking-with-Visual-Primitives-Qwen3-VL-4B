...
def _generate_existence_question(
    ...
    if exists:
        ...
        grounding_parts = [
            f"the {desc} is at {format_box([target.normalized_bbox(img_w, img_h)])}"
        ]
...
def _generate_compare_question(
    ...
    grounding_parts = []
    if boxes1:
        grounding_parts.append(f"{color1} {shape1}s are at {format_box(boxes1)}")
    if boxes2:
        grounding_parts.append(f"{color2} {shape2}s are at {format_box(boxes2)}")
...
def _generate_query_material_question(
    ...
    grounding_parts = [
        ...
        f"{format_box([target.normalized_bbox(img_w, img_h)])}"
    ]
...
def _generate_multihop_question(
    ...
    grounding_parts = [
        f"the {anchor_color} {anchor_shape} is at {format_box([anchor.normalized_bbox(img_w, img_h)])}",
        f"the {target_shape} {direction} it is at {format_box([target.normalized_bbox(img_w, img_h)])}",
    ]
...
def _generate_negative_existence_question(
    ...
    grounding_parts = [
        f"I see {format_box([o.normalized_bbox(img_w, img_h)])} for a {o.color} {o.shape}."
